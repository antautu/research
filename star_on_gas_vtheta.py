### Plots x vs y position graphs with vtheta color bar for gas ###
### Left graph is just vtheta coloring and right graph is vtheta coloring with blue position of stars on top to make it easier to see the spiral arms ###
### [0] = Gas ###
### [4] = Star ###
...
import glio
s = glio.GadgetSnapshot('snapshot_008')		### Change snapshot here ###
s.load()
...
plt.subplot(121)
plt.hexbin(gas_px_xy, gas_py_xy, C=gas_vtheta_bar, cmap=cm.jet_r, gridsize=300, vmin=200, vmax=240)
plt.title('Gas x vs y')
plt.xlabel('x')
plt.ylabel('y')
plt.text(20, 20, 't = 8')				### Make sure to change time label ###
plt.axis([-25, 25, -25, 25])
plt.gca().set_aspect('equal', adjustable='box')
cbar = plt.colorbar()
cbar.set_label('Vtheta')
plt.grid()

plt.subplot(122)
plt.hexbin(gas_px_xy, gas_py_xy, C=gas_vtheta_bar, cmap=cm.jet_r, gridsize=300, vmin=200, vmax=240)
plt.plot(star_px_xy, star_py_xy, '.k', markersize=3, alpha=0.3)
plt.title('Gas x vs y')
plt.xlabel('x')
plt.ylabel('y')
plt.text(20, 20, 't = 8')				### Make sure to change time label ###
