...
### Produces data to plot star x vs y ###
...
### Plots the star and gas x vs y graphs pn the same canvas ###
plt.subplot(121)
plt.plot(star_px_xy, star_py_xy, '.', markersize=3, alpha=0.3)
plt.title('Star x vs y', fontsize=30)
plt.xlabel('x (kpc)', fontsize=28)
plt.ylabel('y (kpc)', fontsize=28)
plt.text(20, 20, 't = 40', fontsize=25)			### Make sure to change time label ###
...
#cbar.set_label('Vz (km/sec)', fontsize=28)
plt.grid()
...
plt.gca().set_aspect('equal', adjustable='box')
#cbar = plt.colorbar(fraction=0.046, pad=0.04, orientation='horizontal')
#cbar.set_label('Vtheta (km/sec)', fontsize=28)
plt.grid()
