...
for i in range(0, len(s.pos[0][:])):
	...
	r_gas = math.sqrt(x_gas**2 + y_gas**2 + z_gas**2)
	theta_gas = math.atan2(y_gas,x_gas)
	phi_gas = math.acos(z_gas / r_gas)

	vx_gas = s.vel[0][i][0]
	vy_gas = s.vel[0][i][1]
	vz_gas = s.vel[0][i][2]

	vr_gas = vx_gas*math.cos(phi_gas)*math.sin(theta_gas) + vy_gas*math.sin(phi_gas)*math.sin(theta_gas) + vz_gas*math.cos(theta_gas)
	vtheta_gas = vx_gas*math.cos(phi_gas)*math.cos(theta_gas) + vy_gas*math.sin(phi_gas)*math.cos(theta_gas) + vz_gas*math.cos(theta_gas)
