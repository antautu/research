### Plots gas position graphs for any snapshot ###
### [0] = Gas ###

import numpy as np
import matplotlib.pyplot as plt
import glio
s = glio.GadgetSnapshot('snapshot_039')		### Change snapshot here ###
s.load()
...
plt.plot(px_xy, py_xy, '.', markersize=3)
plt.xlabel('x')
plt.ylabel('y')
...
plt.plot(py_yz, pz_yz, '.', markersize=3)
plt.xlabel('y')
plt.ylabel('z')
plt.text(20, 3, 't = 39')			### Make sure to change time label ###
plt.axis([-30, 30, -4, 4])
...
plt.plot(px_xz, pz_xz, '.', markersize=3)
plt.xlabel('x')
plt.ylabel('z')
plt.text(20, 3, 't = 39')			### Make sure to change time label ###		
plt.axis([-30, 30, -4, 4])
