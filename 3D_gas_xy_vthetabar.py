### Produces a 3D gas position plot with a Vtheta bar for any snapshot  ###
...
for i in range(0, len(s.pos[0][:])):
	...
	f.write("%s %s %s %s\n" % (x_gas, y_gas, z_gas, vtheta_gas))

f.close()


### Plots a 3D graph of gas position with a Vtheta z axis and color bar ###
...
for line in lines:
	...
	gas_z_xy.append(float(p[2]))
	gas_vtheta_xy.append(float(p[3]))

gas_px_xy = np.array(gas_x_xy)
gas_py_xy = np.array(gas_y_xy)
gas_pz_xy = np.array(gas_z_xy)
gas_vtheta_bar = np.array(gas_vtheta_xy)


fig = plt.figure()
ax = fig.add_subplot(111, projection = '3d')
p = ax.scatter3D(gas_px_xy, gas_py_xy, gas_pz_xy, c=gas_vtheta_bar, cmap='jet_r', s=10, vmin=-150, vmax=240)		### s is the size of the marker ###
cbar = fig.colorbar(p)
ax.set_xlim3d(-25, 25)
ax.set_ylim3d(-25, 25)
ax.set_zlim3d(-40, 40)
